			...
			async toPassAsync(this: QUnitAssert, checkable: Checkable, message?: string) {
				if (!checkable) {
					this.pushResult({
						...
						message: message || 'expected something checkable as an argument for toPassAsync()'
					});
					return;
				}
...
export async function expectToPassAsync(checkable: Checkable, assert: QUnitAssert, message?: string) {
	if (!checkable) {
		assert.ok(false, message || 'expected something checkable as an argument for expectToPassAsync()');
		return;
	}
	const violations = await checkable.check();
	const testViolations = violations.map((v) => ViolationFactory.from(v));
